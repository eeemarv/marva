			...
			$err = $client->getError();
				...
				switch ($result){
					...
					case "OFFLINE":
						...
						log_event("", "Soap", "Remote eLAS/Marva offline $transid");
						break;
					case "FAILED":
					...
					default:
						...
						// Disable message expiration but warn an admin in case of trouble.
...
function localcommit($myletsgroup, $transid, $id_from, $amount, $description, $letscode_to){
	...
	if($mytransid == $transid){
		...
		$systemtag = readconfigfromdb("systemtag");
                $mailsubject .= "[Marva-".$systemtag."] " . "Interlets FAILURE!";
