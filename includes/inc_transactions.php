...
function insert_transaction($posted_list, $transid){
	...
	if($db->AutoExecute("transactions", $posted_list, 'INSERT') == TRUE){
		setstatus("Transactie toegevoegd", 'success');
		...
		setstatus("Fout: Transactie niet toegevoegd", 'danger'); 
		$transid = "";
	}
...
function mail_failed_interlets($myletsgroup, $transid, $id_from, $amount, $description, $letscode_to, $result,$admincc){
	...
	switch ($result){
		case "SIGFAIL":
			$mailcontent .= "De digitale handtekening was ongeldig, dit wijst op een fout in de instellingen van de 2 eLAS/Marva installlatie.  Deze melding werd ook naar de site-beheerder verstuurd.\r\n";
			break;
		case "EXPIRED":
			$mailcontent .= "Na 4 dagen kon geen contact met de andere eLAS/Marva installatie gemaakt worden, probeer de transactie later opnieuw of verwittig de beheerder als dit blijft.\r\n";
			break;
		case "NOUSER":
...
function queuetransaction($posted_list) {
	...
	if($db->AutoExecute("interletsq", $posted_list, 'INSERT') == TRUE){
                setstatus("Transactie in wachtrij");
		$transid = $posted_list["transid"];
        } else {
                setstatus("Fout: Transactie niet opgeslagen", 'danger');
                $transid = "";
        }
