...
function show_form($id,$email,$error_list,$posted_list){
	...
	if(!empty($error_list["login"])){
        ...
        echo "</td></tr>";
        echo "<tr><td>";
        echo "E-mail adres<br><small><i>(Dit adres moet geregistreerd staan)</i></small>";
        echo "</td><td>";
	if(isset($email)){
...
function validate_input($posted_list){
        $error_list = array();
	if(empty($posted_list["login"])){
	                $error_list["login"] = "<font color='red'> Vul een <strong>Marva login</strong> in</font>";
	}
	if(empty($posted_list["email"])){
	...
	if(empty($checkedaddress)) {
			$error_list["email"] = "<font color='red'> Dit mailadres is niet gekend in Marva</font>";
	}
	if(empty($posted_list["subject"])){
