...
if(s_accountrole != "admin" && $s_letscode != $_POST["letscode_from"]) {
	...
	echo "Transactie niet toegestaan (foute letscode en geen admin - " .$s_letscode ." - " .$s_accountrole ." - "  .$_POST["letscode_from"] " - )";
	exit;
}
	...
	case "internal":
		...
		if(!empty($errors)){
			...
			if($mytransid == $settransid){
				...
				setstatus("Gefaalde transactie", 'danger');
			}
		}
		break;
	case "mail":
